...
Windows + Android first, eventual Steam distribution, iOS much later. One cross-platform player population. React + TypeScript + Canvas with Electron/Capacitor packaging, not Unity. Large phases and Spark-first implementation to conserve regular model usage. Headless terminal testing without taking over the user's desktop.
...
Record date, request/evidence, old decision, new decision, affected modules/spec extracts/tests, and owner approval when required. Preserve the master plan as the authoritative specification.
